...
function svgIcon2(name) {
  ...
  return `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">${paths[name]}</svg>`;
}
...
function aplicarConfiguracion(s) {
  ...
  document.documentElement.style.setProperty('--text', s.color_texto || '#1B1B1F');
  document.documentElement.style.setProperty('--icon-color', s.color_iconos || '#14161F');
  document.documentElement.style.setProperty('--header-text', s.color_header_texto || '#FFFFFF');
  document.documentElement.style.setProperty('--menu-text', s.color_menu_texto || '#D5D6E0');

  // Fondos del header/footer y de botones: sólidos o degradados
  const bgHeader = (s.color_primary_gradient && s.color_primary_gradient.activo)
    ? `linear-gradient(${s.color_primary_gradient.angulo}deg, ${s.color_primary}, ${s.color_primary_gradient.color2})`
    : s.color_primary;
  const bgAccent = (s.color_accent_gradient && s.color_accent_gradient.activo)
    ? `linear-gradient(${s.color_accent_gradient.angulo}deg, ${s.color_accent}, ${s.color_accent_gradient.color2})`
    : s.color_accent;
  document.documentElement.style.setProperty('--bg-header', bgHeader);
  document.documentElement.style.setProperty('--bg-accent', bgAccent);
  ...
  document.getElementById('logoImg').style.height = (s.logo_size || 36) + 'px';

  const favicon = document.getElementById('faviconLink');
  if (s.favicon_url) favicon.href = s.favicon_url;

  // Derechos de autor en el encabezado
  const copyEl = document.getElementById('headerCopyright');
  if (s.header_copyright) { copyEl.textContent = s.header_copyright; copyEl.style.display = 'block'; }
  else copyEl.style.display = 'none';

  // Color de fondo de la barra de categorías
  document.getElementById('catnav').style.background = s.color_categorias || '';

  renderHeroSlides(s);

  // Banner del carrito (opcional)
  const cartBannerWrap = document.getElementById('cartBannerWrap');
  if (s.carrito_banner_imagen) {
    cartBannerWrap.innerHTML = `
      <a class="cart-banner" href="${s.carrito_banner_link || '#'}" target="${s.carrito_banner_link ? '_blank' : '_self'}">
        <img src="${s.carrito_banner_imagen}" alt="">
      </a>`;
  } else {
    cartBannerWrap.innerHTML = '';
  }
  ...
  document.getElementById('bannersRow').innerHTML = banners.map(b => `
    <a class="banner-block ${b.ancho_completo ? 'ancho-completo' : ''}" href="${b.link || '#'}" target="${b.link ? '_blank' : '_self'}">
  ...
  const TIPO_ICONO_COUNTDOWN = { oferta: 'star', envio_gratis: 'truck', descuento: 'box', promocion: 'heart' };
  const cd = s.countdown || {};
  clearInterval(window._countdownInterval);
  const bar = document.getElementById('countdownBar');
  if (cd.activo && cd.fecha_fin && new Date(cd.fecha_fin) > new Date()) {
    bar.style.display = 'flex';
    const iconKey = TIPO_ICONO_COUNTDOWN[cd.tipo] || 'star';
    document.getElementById('countdownTexto').innerHTML =
      `<span class="anim-${cd.animacion || 'none'}" style="display:inline-flex;vertical-align:middle;margin-right:6px;">${svgIcon(iconKey, 14)}</span>${cd.texto || ''}`;
  ...
  aplicarCamposObligatorios(s);
  aplicarConfiguracionRuleta(s.ruleta);
...
function renderHeroSlides(s) {
  const heroEl = document.getElementById('hero');
  heroEl.classList.remove('altura-full', 'altura-media', 'solo-imagen-modo');
  heroEl.style.height = '';
  if (s.hero_altura === 'media') heroEl.classList.add('altura-media');
  else if (s.hero_altura === 'personalizada') heroEl.style.height = (s.hero_altura_px || 480) + 'px';
  else heroEl.classList.add('altura-full');
  heroEl.classList.toggle('ocultar-imagen-movil', !!s.hero_ocultar_imagen_movil);

  let slides = (s.hero_slides || []).filter(sl => sl.image || sl.titulo || sl.subtitulo);
  if (!slides.length && (s.hero_title || s.hero_image)) {
    slides = [{ image: s.hero_image, titulo: s.hero_title, subtitulo: s.hero_subtitle }];
  }
  if (!slides.length) slides = [{ image: '', titulo: 'Compra sin miedo.\nPaga cuando lo tengas en tus manos.', subtitulo: '' }];

  const opacidad = (s.hero_opacidad_imagen !== undefined ? s.hero_opacidad_imagen : 100) / 100;
  const oscurecer = (s.hero_oscurecer_imagen !== undefined ? s.hero_oscurecer_imagen : 40) / 100;
  const mostrarTexto = s.hero_mostrar_texto !== false;
  ...
  track.innerHTML = slides.map((sl, i) => {
    const soloImagen = !mostrarTexto || (!sl.titulo && !sl.subtitulo);
    return `
      <div class="hero-slide ${i === 0 ? 'active' : ''} ${soloImagen ? 'solo-imagen-slide' : ''}">
        ${sl.image ? `<img class="slide-bg" src="${sl.image}" style="opacity:${opacidad}">` : ''}
        ${sl.image ? `<div class="slide-overlay" style="background:rgba(20,22,31,${oscurecer});"></div>` : ''}
        <div class="hero-inner">
          ${sl.titulo ? `<h1>${sl.titulo.replace(/\n/g, '<br>')}</h1>` : ''}
  ...
  dotsWrap.innerHTML = multiple ? slides.map((_, i) => `<span class="${i === 0 ? 'active' : ''}" data-i="${i}"></span>`).join('') : '';

  const irASlide = (i) => {
    window._heroIndex = (i + slides.length) % slides.length;
    track.querySelectorAll('.hero-slide').forEach((el, idx) => el.classList.toggle('active', idx === window._heroIndex));
    dotsWrap.querySelectorAll('span').forEach((el, idx) => el.classList.toggle('active', idx === window._heroIndex));
  };
  prevBtn.onclick = () => irASlide(window._heroIndex - 1);
  nextBtn.onclick = () => irASlide(window._heroIndex + 1);
  dotsWrap.querySelectorAll('span').forEach(dot => dot.onclick = () => irASlide(parseInt(dot.dataset.i)));

  clearInterval(window._heroAutoplayInterval);
  if (multiple && s.hero_autoplay !== false) {
    const segundos = s.hero_intervalo_segundos || 5;
    window._heroAutoplayInterval = setInterval(() => irASlide(window._heroIndex + 1), segundos * 1000);
  }
}

// ================= CAMPOS OBLIGATORIOS DEL CHECKOUT =================
function aplicarCamposObligatorios(s) {
  const form = document.getElementById('checkoutForm');
  if (!form) return;
  const obligatorios = s.campos_obligatorios || {};
  const SIEMPRE_OBLIGATORIOS = ['nombre_cliente', 'telefono', 'direccion'];
  ['nombre_cliente', 'telefono', 'direccion', 'ciudad', 'departamento', 'notas_cliente'].forEach(campo => {
    const input = form.querySelector(`[name="${campo}"]`);
    if (!input) return;
    input.required = SIEMPRE_OBLIGATORIOS.includes(campo) ? true : !!obligatorios[campo];
  });
}
  ...
  if (popup.tipo === 'steps') {
    html += popup.items.map(item => `
      <div class="popup-step">
        <div class="step-icon">${item.imagen ? `<img src="${item.imagen}" class="anim-${item.animacion || 'none'}">` : `<span class="anim-${item.animacion || 'none'}">${svgIcon(item.icono || 'check', 22)}</span>`}</div>
        <div>
          <p class="step-title anim-${item.animacion_texto || 'none'}">${item.titulo}</p>
          <p class="step-text anim-${item.animacion_texto || 'none'}">${item.texto}</p>
        </div>
      </div>
    `).join('');
    ...
    html += popup.items.map(item => `
      <div class="popup-testimonial">
        <div class="t-avatar">${item.imagen ? `<img src="${item.imagen}" class="anim-${item.animacion || 'none'}">` : `<span class="anim-${item.animacion || 'none'}">${svgIcon(item.icono || 'smile', 20)}</span>`}</div>
        <div>
          <p class="t-name anim-${item.animacion_texto || 'none'}">${item.titulo}</p>
          <p class="t-text anim-${item.animacion_texto || 'none'}">${item.texto}</p>
        </div>
      </div>
    `).join('');
  } else {
...
function renderCategorias() {
  ...
  // Muchas categorías: mostramos las primeras como chips y el resto en un menú "Más"
  const visibles = cats.slice(0, LIMITE_VISIBLE);
  const resto = cats.slice(LIMITE_VISIBLE);
  const restoActiva = resto.includes(CATEGORIA_ACTIVA);

  cont.innerHTML = visibles.map(c =>
    `<button class="chip ${c === CATEGORIA_ACTIVA ? 'active' : ''}" data-cat="${c}">${c}</button>`
  ).join('') + `
    <div class="catnav-more-wrap">
      <button class="catnav-more-btn ${restoActiva ? 'active' : ''}" id="catnavMoreBtn">
        ${restoActiva ? CATEGORIA_ACTIVA : 'Más'} ${svgIcon2('chevronDown')}
      </button>
      <div class="catnav-more-dropdown" id="catnavMoreDropdown">
        ${resto.map(c => `<button class="${c === CATEGORIA_ACTIVA ? 'active' : ''}" data-cat="${c}">${c}</button>`).join('')}
      </div>
    </div>
  `;
  ...
  const moreBtn = document.getElementById('catnavMoreBtn');
  const dropdown = document.getElementById('catnavMoreDropdown');
  moreBtn.onclick = (e) => {
    e.stopPropagation();
    ...
    dropdown.classList.toggle('open', abriendo);
    moreBtn.classList.toggle('open', abriendo);
  };
  dropdown.querySelectorAll('button').forEach(el => {
    el.onclick = () => { CATEGORIA_ACTIVA = el.dataset.cat; renderCategorias(); renderCatalogo(); };
  });
  document.addEventListener('click', () => { dropdown.classList.remove('open'); moreBtn.classList.remove('open'); }, { once: true });
}
...
function renderCatalogo() {
  ...
  const buscando = TEXTO_BUSQUEDA.trim().length > 0;

  // Mientras se busca, ocultamos el hero y los banners para que solo se vean los resultados
  document.getElementById('bannersRow').style.display = buscando ? 'none' : '';
  document.getElementById('hero').style.display = buscando ? 'none' : '';

  document.getElementById('catalogTitle').textContent = buscando
  ...
  // Banners intercalados: se insertan después de la posición configurada (solo si no se está buscando)
  const banners = (!buscando && SETTINGS && SETTINGS.banners_intercalados) ? SETTINGS.banners_intercalados.filter(b => b.imagen) : [];
  const bannerHTML = (b) => `
    <div class="banner-intercalado-wrap">
      <a href="${b.link || '#'}" target="${b.link ? '_blank' : '_self'}"><img src="${b.imagen}" alt=""></a>
    </div>`;

  // Insertamos de atrás hacia adelante para que las posiciones no se corran entre sí
  const ordenados = [...banners].sort((a, b) => b.posicion - a.posicion);
  ordenados.forEach(b => {
    const idx = Math.min(Math.max(b.posicion, 0), tarjetas.length);
    tarjetas.splice(idx, 0, bannerHTML(b));
  });
...
function abrirProducto(id) {
  ...
  const html = `
    ...
    <div class="reviews-section" id="reviewsSection">
      ...
      <form class="review-form" id="reviewForm">
        ...
        <button type="submit" class="btn-review-submit">${svgIcon('check', 15)} Enviar comentario</button>
  ...
  if (!agotado) {
    ...
    document.getElementById('buyNowBtn').onclick = () => {
      ...
      actualizarVisibilidadCajaDescuento();
    };
  }
...
document.getElementById('goCheckout').onclick = () => { cerrarModales(); abrir('checkoutOverlay'); renderResumenCheckout(); actualizarVisibilidadCajaDescuento(); };
...
function renderResumenCheckout() {
  ...
  const etiquetaDescuento = DESCUENTO_ACTIVO
    ? (DESCUENTO_ACTIVO.origen === 'ruleta' ? 'Descuento de la ruleta' : `Descuento (${DESCUENTO_ACTIVO.codigo})`)
    : '';
  document.getElementById('checkoutTotalSummary').innerHTML = `
    <div class="line"><span>Subtotal</span><span>${fmt(subtotal)}</span></div>
    ${descuento > 0 ? `<div class="line"><span>${etiquetaDescuento}</span><span>-${fmt(descuento)}</span></div>` : ''}
    <div class="line total"><span>Total</span><span>${fmt(total)}</span></div>
  `;
}

// Mientras ya hay un descuento activo (de la ruleta o manual), se oculta la caja para meter otro código
function actualizarVisibilidadCajaDescuento() {
  const wrap = document.getElementById('discountBoxWrap');
  const activoMsg = document.getElementById('descuentoActivoMsg');
  if (DESCUENTO_ACTIVO) {
    wrap.style.display = 'none';
    activoMsg.style.display = 'block';
    activoMsg.textContent = DESCUENTO_ACTIVO.origen === 'ruleta'
      ? `🎉 Ya tienes ${DESCUENTO_ACTIVO.porcentaje}% de descuento aplicado (ganado en la ruleta, válido por 10 minutos).`
      : `Código "${DESCUENTO_ACTIVO.codigo}" aplicado (${DESCUENTO_ACTIVO.porcentaje}% de descuento).`;
  } else {
    wrap.style.display = 'block';
    activoMsg.style.display = 'none';
  }
}
...
document.getElementById('applyDiscountBtn').onclick = async () => {
  ...
  try {
    ...
    DESCUENTO_ACTIVO = { codigo: data.codigo, porcentaje: data.porcentaje, origen: 'manual' };
...
document.getElementById('checkoutForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const confirmBannerWrap = document.getElementById('confirmBannerWrap');
    confirmBannerWrap.innerHTML = (SETTINGS && SETTINGS.confirmacion_imagen)
      ? `<div class="confirm-banner"><img src="${SETTINGS.confirmacion_imagen}" alt=""></div>` : '';
    CARRITO = [];
    DESCUENTO_ACTIVO = null;
    clearTimeout(window._ruletaDescuentoTimeout);
...
let RULETA_SEGMENTOS = [];
let RULETA_ROTACION_ACTUAL = 0;
let RULETA_GIRANDO = false;
let RULETA_COOLDOWN_INTERVAL = null;

function aplicarConfiguracionRuleta(r) {
  const btn = document.getElementById('ruletaBtn');
  if (!r || !r.activa) { btn.style.display = 'none'; return; }
  btn.style.display = 'flex';
  document.getElementById('ruletaBtnTexto').textContent = r.boton_texto || 'Gana descuentos';
  document.getElementById('ruletaEyebrow').textContent = r.eyebrow || '';
  document.getElementById('ruletaTitulo').textContent = r.titulo || '';
  document.getElementById('ruletaPie').textContent = r.pie_texto || '';

  const modal = document.querySelector('.ruleta-modal');
  modal.style.setProperty('--r-fondo', r.color_fondo || '#2A0944');
  modal.style.setProperty('--r-acento', r.color_acento || '#FF2D6F');
  modal.style.setProperty('--r-dorado', r.color_dorado || '#FFD23F');
  modal.style.setProperty('--r-texto', r.color_texto || '#FFF8F0');

  RULETA_SEGMENTOS = (r.segmentos && r.segmentos.length) ? r.segmentos : [{ texto: '0%', color: '#2A0944', premio: false, codigo: '' }];
  construirRuedaSVG();
}

function construirRuedaSVG() {
  const svg = document.getElementById('ruletaSvg');
  svg.innerHTML = '';
  const segCount = RULETA_SEGMENTOS.length;
  ...
  RULETA_SEGMENTOS.forEach((seg, i) => {
    const start = i * segAngle;
    const end = start + segAngle;
    const p1 = polar(150, 150, radius, start);
    const p2 = polar(150, 150, radius, end);
    const largeArc = segAngle > 180 ? 1 : 0;

    const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    path.setAttribute('d', `M150,150 L${p1.x},${p1.y} A${radius},${radius} 0 ${largeArc} 1 ${p2.x},${p2.y} Z`);
    path.setAttribute('fill', seg.color || '#2A0944');
    svg.appendChild(path);

    const mid = start + segAngle / 2;
    const textPos = polar(150, 150, radius * 0.62, mid);
    const text = document.createElementNS('http://www.w3.org/2000/svg', 'text');
    text.setAttribute('x', textPos.x);
    text.setAttribute('y', textPos.y);
    text.setAttribute('fill', '#FFF8F0');
    text.setAttribute('font-weight', '800');
    text.setAttribute('font-size', '24');
    text.setAttribute('text-anchor', 'middle');
    text.setAttribute('dominant-baseline', 'middle');
    text.setAttribute('transform', `rotate(${mid}, ${textPos.x}, ${textPos.y})`);
    text.textContent = seg.texto || '';
    svg.appendChild(text);
  });
}

document.getElementById('ruletaBtn').onclick = () => {
  document.getElementById('ruletaStage').style.display = 'flex';
...
async function girarRuleta() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (data.restante) iniciarCooldownRuleta(data.restante);
      else alert(data.error || 'No se pudo girar la ruleta.');
      return;
    }
  } catch (e) {
    RULETA_GIRANDO = false;
    alert('Ocurrió un error al girar. Intenta de nuevo.');
    return;
  }
  ...
  const currentMod = ((RULETA_ROTACION_ACTUAL % 360) + 360) % 360;

  let delta = desiredMod - currentMod;
  if (delta <= 0) delta += 360;

  const extraSpins = 6 * 360;
  RULETA_ROTACION_ACTUAL += extraSpins + delta;

  const wheel = document.getElementById('ruletaWheel');
  wheel.style.transform = `rotate(${RULETA_ROTACION_ACTUAL}deg)`;

  const onEnd = () => {
    wheel.removeEventListener('transitionend', onEnd);
    mostrarResultadoRuleta(data);
    iniciarCooldownRuleta();
    RULETA_GIRANDO = false;
  };
  wheel.addEventListener('transitionend', onEnd);
}
document.getElementById('ruletaSpinBtn').onclick = girarRuleta;

function mostrarResultadoRuleta(data) {
  document.getElementById('ruletaStage').style.display = 'none';
  const resultBox = document.getElementById('ruletaResult');
  resultBox.style.display = 'block';
  const icon = document.getElementById('ruletaResultIcon');

  if (data.premio) {
    icon.innerHTML = svgIcon('check', 24);
    document.getElementById('ruletaResultTitle').textContent = `¡Ganaste ${data.texto} de descuento!`;
    document.getElementById('ruletaResultSub').textContent = 'Ya se aplicó automáticamente en tu carrito — dura 10 minutos.';
    aplicarDescuentoDeRuleta(data.codigo, data.porcentaje);
    lanzarConfetiRuleta();
  } else {
    icon.innerHTML = svgIcon('heart', 24);
    document.getElementById('ruletaResultTitle').textContent = '¡Casi lo logras!';
    document.getElementById('ruletaResultSub').textContent = 'Esta vez no hubo descuento. ¡Vuelve a intentarlo en tu próximo giro!';
  }
}

// Aplica el descuento ganado directamente (sin que el cliente vea ningún código) y lo hace vencer en 10 minutos
function aplicarDescuentoDeRuleta(codigo, porcentaje) {
  DESCUENTO_ACTIVO = { codigo, porcentaje, origen: 'ruleta' };
  clearTimeout(window._ruletaDescuentoTimeout);
  window._ruletaDescuentoTimeout = setTimeout(() => {
    if (DESCUENTO_ACTIVO && DESCUENTO_ACTIVO.codigo === codigo) {
      DESCUENTO_ACTIVO = null;
      renderResumenCheckout();
      actualizarVisibilidadCajaDescuento();
    }
...
document.getElementById('ruletaVolverBtn').onclick = () => {
  document.getElementById('ruletaResult').style.display = 'none';
  document.getElementById('ruletaStage').style.display = 'flex';
};

function iniciarCooldownRuleta(segundosIniciales) {
  const segundos = segundosIniciales || (SETTINGS && SETTINGS.ruleta && SETTINGS.ruleta.cooldown_segundos) || 30;
  let restante = segundos;
  const btn = document.getElementById('ruletaSpinBtn');
  btn.disabled = true;
  btn.textContent = `Espera ${restante}s...`;
  clearInterval(RULETA_COOLDOWN_INTERVAL);
  RULETA_COOLDOWN_INTERVAL = setInterval(() => {
    restante -= 1;
    if (restante <= 0) {
      clearInterval(RULETA_COOLDOWN_INTERVAL);
      btn.disabled = false;
...
function lanzarConfetiRuleta() {
  const colores = ['#FF2D6F', '#FFD23F', '#6A2C91', '#16C784'];
  for (let i = 0; i < 34; i++) {
