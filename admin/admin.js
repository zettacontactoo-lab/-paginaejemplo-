...
async function cargarRuleta() {
  ...
  RULETA_SEGMENTOS_ADMIN = (r.segmentos && r.segmentos.length) ? JSON.parse(JSON.stringify(r.segmentos)) : [{ texto: '', color: '#2A0944', premio: false, codigo: '' }];
...
function renderRuletaSegmentosEditor() {
  document.getElementById('ruletaSegmentosEditor').innerHTML = RULETA_SEGMENTOS_ADMIN.map((seg, i) => `
    <div class="hero-slide-box">
      ...
      <label class="checkbox-row"><input type="checkbox" class="rlseg-premio" data-i="${i}" ${seg.premio ? 'checked' : ''}> Este sector es un premio (da un código de descuento)</label>
      <label class="rlseg-codigo-wrap" style="display:${seg.premio ? 'flex' : 'none'};">Código de descuento
        <input type="text" class="rlseg-codigo" data-i="${i}" value="${(seg.codigo || '').replace(/"/g, '&quot;')}" placeholder="Ej: BIENVENIDA15">
      </label>
    </div>
  `).join('');
  ...
  document.querySelectorAll('.rlseg-codigo').forEach(inp => {
    inp.addEventListener('input', () => { RULETA_SEGMENTOS_ADMIN[inp.dataset.i].codigo = inp.value; });
  });
...
document.getElementById('addRuletaSegmentoBtn').onclick = () => {
  RULETA_SEGMENTOS_ADMIN.push({ texto: '', color: '#2A0944', premio: false, codigo: '' });
  renderRuletaSegmentosEditor();
};
