...
document.querySelectorAll('.nav-item[data-tab]').forEach(item => {
  item.onclick = () => {
    ...
    if (item.dataset.tab === 'ruleta') cargarRuleta();
  };
});
...
// ================= BANNER ENTRE PRODUCTOS (intercalado) =================
function renderBannerIntercaladoEditor() {
  const lista = PREVIEW.banners_intercalados && PREVIEW.banners_intercalados.length ? PREVIEW.banners_intercalados : [];
  document.getElementById('bannerIntercaladoEditor').innerHTML = lista.map((b, i) => `
    <div class="panel-box" style="background:#FAFAFB;margin-bottom:10px;position:relative;">
      <button type="button" class="banner-remove-btn" data-birm="${i}" title="Quitar este banner">
        <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
      </button>
      <h3>Banner entre productos ${i + 1}</h3>
      <div class="form-grid">
        <label>Imagen (se muestra tal cual, sin recortar ni oscurecer)
          <input type="file" accept="image/*" class="bi-img-input" data-i="${i}">
        </label>
        <div class="img-preview-row" id="biPreview_${i}">${b.imagen ? `<img src="${b.imagen}">` : ''}</div>
        <label>Aparece después de cuántos productos
          <input type="number" min="0" class="bi-posicion-input" data-i="${i}" value="${b.posicion || 4}">
        </label>
        <label>Link al hacer clic (opcional)
          <input type="text" class="bi-link-input" data-i="${i}" value="${b.link || ''}" placeholder="https://...">
        </label>
      </div>
    </div>
  `).join('') || '<p class="hint">Aún no has agregado ninguno.</p>';

  document.querySelectorAll('[data-birm]').forEach(btn => {
    btn.onclick = () => { PREVIEW.banners_intercalados.splice(parseInt(btn.dataset.birm), 1); renderBannerIntercaladoEditor(); };
  });
  document.querySelectorAll('.bi-img-input').forEach(input => {
    input.addEventListener('change', (e) => {
      const i = input.dataset.i;
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = (ev) => {
        document.getElementById(`biPreview_${i}`).innerHTML = `<img src="${ev.target.result}">`;
        PREVIEW.banners_intercalados[i].imagen = ev.target.result;
        PREVIEW.banners_intercalados[i]._file = file;
      };
      reader.readAsDataURL(file);
    });
  });
  document.querySelectorAll('.bi-posicion-input').forEach(inp => {
    inp.addEventListener('input', () => { PREVIEW.banners_intercalados[inp.dataset.i].posicion = parseInt(inp.value) || 0; });
  });
  document.querySelectorAll('.bi-link-input').forEach(inp => {
    inp.addEventListener('input', () => { PREVIEW.banners_intercalados[inp.dataset.i].link = inp.value; });
  });
}
document.getElementById('addBannerIntercaladoBtn').onclick = () => {
  if (!PREVIEW.banners_intercalados) PREVIEW.banners_intercalados = [];
  PREVIEW.banners_intercalados.push({ imagen: '', posicion: 4, link: '' });
  renderBannerIntercaladoEditor();
};
...
document.getElementById('settingsForm').addEventListener('submit', async (e) => {
  ...
  // Banners entre productos (intercalados)
  const bannersIntercaladosMeta = (PREVIEW.banners_intercalados || []).map(b => ({
    imagen: b.imagen && b.imagen.startsWith('data:') ? '' : b.imagen, posicion: b.posicion, link: b.link
  }));
  fd.set('banners_intercalados_json', JSON.stringify(bannersIntercaladosMeta));
  (PREVIEW.banners_intercalados || []).forEach((b, i) => {
    if (b._file) fd.append(`banner_intercalado_image_${i}`, b._file);
  });
...
async function cargarRuleta() {
  ...
  document.getElementById('rl_activa').checked = !!r.activa;
  document.getElementById('rl_boton_texto').value = r.boton_texto || '';
  document.getElementById('rl_eyebrow').value = r.eyebrow || '';
  document.getElementById('rl_titulo').value = r.titulo || '';
  document.getElementById('rl_pie_texto').value = r.pie_texto || '';
  document.getElementById('rl_cooldown').value = r.cooldown_segundos || 30;
  document.getElementById('rl_color_fondo').value = r.color_fondo || '#2A0944';
  document.getElementById('rl_color_acento').value = r.color_acento || '#FF2D6F';
  document.getElementById('rl_color_dorado').value = r.color_dorado || '#FFD23F';
  document.getElementById('rl_color_texto').value = r.color_texto || '#FFF8F0';

  RULETA_SEGMENTOS_ADMIN = (r.segmentos && r.segmentos.length) ? JSON.parse(JSON.stringify(r.segmentos)) : [{ texto: '', color: '#2A0944', premio: false, porcentaje: 0 }];
  renderRuletaSegmentosEditor();
}

function renderRuletaSegmentosEditor() {
  document.getElementById('ruletaSegmentosEditor').innerHTML = RULETA_SEGMENTOS_ADMIN.map((seg, i) => `
    <div class="hero-slide-box">
      <h4>Sector ${i + 1}</h4>
      ${RULETA_SEGMENTOS_ADMIN.length > 1 ? `<button type="button" class="banner-remove-btn" data-segrm="${i}" title="Quitar este sector">
        <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
      </button>` : ''}
      <div class="row2">
        <label>Texto en la ruleta
          <input type="text" class="rlseg-texto" data-i="${i}" value="${(seg.texto || '').replace(/"/g, '&quot;')}" placeholder="Ej: 15%">
        </label>
        <label>Color del sector
          <input type="color" class="rlseg-color" data-i="${i}" value="${seg.color || '#2A0944'}">
        </label>
      </div>
      <label class="checkbox-row"><input type="checkbox" class="rlseg-premio" data-i="${i}" ${seg.premio ? 'checked' : ''}> Este sector es un premio (aplica un descuento directo)</label>
      <label class="rlseg-codigo-wrap" style="display:${seg.premio ? 'flex' : 'none'};">Porcentaje de descuento
        <input type="number" min="1" max="100" class="rlseg-porcentaje" data-i="${i}" value="${seg.porcentaje || ''}" placeholder="Ej: 15">
      </label>
    </div>
  `).join('');

  document.querySelectorAll('.rlseg-texto').forEach(inp => {
    inp.addEventListener('input', () => { RULETA_SEGMENTOS_ADMIN[inp.dataset.i].texto = inp.value; });
  });
  document.querySelectorAll('.rlseg-color').forEach(inp => {
    inp.addEventListener('input', () => { RULETA_SEGMENTOS_ADMIN[inp.dataset.i].color = inp.value; });
  });
  document.querySelectorAll('.rlseg-porcentaje').forEach(inp => {
    inp.addEventListener('input', () => { RULETA_SEGMENTOS_ADMIN[inp.dataset.i].porcentaje = parseInt(inp.value) || 0; });
  });
  document.querySelectorAll('.rlseg-premio').forEach(chk => {
    chk.addEventListener('change', () => {
      RULETA_SEGMENTOS_ADMIN[chk.dataset.i].premio = chk.checked;
      renderRuletaSegmentosEditor();
    });
  });
  document.querySelectorAll('[data-segrm]').forEach(btn => {
    btn.onclick = () => {
      RULETA_SEGMENTOS_ADMIN.splice(parseInt(btn.dataset.segrm), 1);
      renderRuletaSegmentosEditor();
    };
  });
}

document.getElementById('addRuletaSegmentoBtn').onclick = () => {
  RULETA_SEGMENTOS_ADMIN.push({ texto: '', color: '#2A0944', premio: false, porcentaje: 0 });
  renderRuletaSegmentosEditor();
};

document.getElementById('ruletaForm').addEventListener('submit', async (e) => {
  e.preventDefault();
  const ruleta = {
    activa: document.getElementById('rl_activa').checked,
    boton_texto: document.getElementById('rl_boton_texto').value,
    eyebrow: document.getElementById('rl_eyebrow').value,
    titulo: document.getElementById('rl_titulo').value,
    pie_texto: document.getElementById('rl_pie_texto').value,
    cooldown_segundos: parseInt(document.getElementById('rl_cooldown').value) || 30,
    color_fondo: document.getElementById('rl_color_fondo').value,
    color_acento: document.getElementById('rl_color_acento').value,
    color_dorado: document.getElementById('rl_color_dorado').value,
    color_texto: document.getElementById('rl_color_texto').value,
    segmentos: RULETA_SEGMENTOS_ADMIN.filter(s => s.texto.trim())
  };

  const fd = new FormData();
  fd.set('ruleta_json', JSON.stringify(ruleta));
  const res = await fetch('/api/admin/settings', { method: 'PUT', body: fd });
  const msg = document.getElementById('ruletaSavedMsg');
  if (res.ok) {
    msg.style.display = 'block';
    setTimeout(() => { msg.style.display = 'none'; }, 4000);
    cargarRuleta();
  } else {
    alert('Ocurrió un error al guardar la ruleta.');
  }
});
