...
function girarRuleta() {
  ...
  const onEnd = () => {
    ...
    mostrarResultadoRuleta(RULETA_SEGMENTOS[idx]);
...
function mostrarResultadoRuleta(seg) {
  ...
  const codeBox = document.getElementById('ruletaCodeBox');

  if (seg.premio && seg.codigo) {
    icon.innerHTML = svgIcon('check', 24);
    document.getElementById('ruletaResultTitle').textContent = `¡Ganaste ${seg.texto} de descuento!`;
    document.getElementById('ruletaResultSub').textContent = 'Usa este código en tu compra:';
    codeBox.style.display = 'flex';
    document.getElementById('ruletaCodeText').textContent = seg.codigo;
    lanzarConfetiRuleta();
  } else {
...
document.getElementById('ruletaCopyBtn').onclick = async () => {
  const text = document.getElementById('ruletaCodeText').textContent;
  const btn = document.getElementById('ruletaCopyBtn');
  try {
...
function iniciarCooldownRuleta() {
  const segundos = (SETTINGS && SETTINGS.ruleta && SETTINGS.ruleta.cooldown_segundos) || 30;
