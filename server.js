...
function parseCSV(text) {
  ...
  // Detectamos el separador mirando solo la primera línea real del encabezado
  const primeraLinea = text.split(/\r?\n/)[0] || '';
  const separador = (primeraLinea.split(';').length > primeraLinea.split(',').length) ? ';' : ',';

  // Parser completo del archivo (no por línea) para que las descripciones con
  // saltos de línea DENTRO de comillas no rompan el resto de las filas.
  const filas = [];
  let fila = [];
  let cur = '';
  ...
  // Última fila si el archivo no terminó con salto de línea
  if (cur.length || fila.length) { fila.push(cur); filas.push(fila); }

  const filasNoVacias = filas.filter(f => f.some(v => v.trim().length));
  if (!filasNoVacias.length) return [];

  const headers = filasNoVacias[0].map(h => h.trim().toLowerCase());
  return filasNoVacias.slice(1).map(valores => {
    const row = {};
...
// Convierte "$64.255", "64,255", "64255" etc. en el número 64255
...
function parseVariantesCSV(texto) {
  ...
  return texto.split(';').map(grupoTexto => {
    ...
    // Sin "Grupo:" al inicio (ej: "Blanco, Negro, Rosado") — se agrupan bajo "Opciones"
    const opciones = grupoTexto.split(',').map(o => o.trim()).filter(Boolean);
    if (!opciones.length) return null;
    return { grupo: 'Opciones', opciones };
...
app.post('/api/admin/products/import', requireAdmin, uploadTmp.single('csv'), (req, res) => {
  ...
  filas.forEach((fila, idx) => {
    ...
    const datos = [
      ...
      parseMoneda(fila.precio_venta),
      fila.precio_oferta ? parseMoneda(fila.precio_oferta) : null,
      parseMoneda(fila.precio_proveedor),
...
app.get('/api/admin/discounts', requireAdmin, (req, res) => {
  res.json(db.prepare('SELECT * FROM discount_codes WHERE generado_por_ruleta = 0 ORDER BY id DESC').all());
});
...
app.post('/api/apply-discount', (req, res) => {
  ...
  if (!code || codigoExpirado(code)) return res.status(404).json({ error: 'Código inválido o inactivo' });
...
// Un código generado por la ruleta vence a los 10 minutos de creado, sin importar si se usó o no
function codigoExpirado(code) {
  if (!code.expira) return false;
  return new Date(code.expira.replace(' ', 'T')).getTime() < Date.now();
}

// ================= RULETA DE DESCUENTOS (el servidor decide el sector, no el navegador) =================
app.post('/api/ruleta/girar', (req, res) => {
  const settings = getSettings();
  const ruleta = settings.ruleta || {};
  if (!ruleta.activa) return res.status(400).json({ error: 'La ruleta no está activa' });

  const segmentos = ruleta.segmentos || [];
  if (!segmentos.length) return res.status(400).json({ error: 'La ruleta no tiene sectores configurados' });

  // Cooldown por sesión: no se puede volver a girar antes de tiempo
  const cooldownSegundos = ruleta.cooldown_segundos || 30;
  if (req.session.ultimoGiroRuleta) {
    const segundosDesdeGiro = (Date.now() - req.session.ultimoGiroRuleta) / 1000;
    if (segundosDesdeGiro < cooldownSegundos) {
      const restante = Math.ceil(cooldownSegundos - segundosDesdeGiro);
      return res.status(429).json({ error: `Debes esperar ${restante} segundos para volver a girar.`, restante });
    }
  }
  req.session.ultimoGiroRuleta = Date.now();

  // El servidor elige el sector al azar — así nadie puede manipular el resultado desde el navegador
  const index = Math.floor(Math.random() * segmentos.length);
  const segmento = segmentos[index];

  const respuesta = { ok: true, index, premio: !!(segmento.premio && segmento.porcentaje > 0), texto: segmento.texto };

  if (respuesta.premio) {
    // Generamos un código interno oculto (el cliente nunca lo ve) que vence en 10 minutos
    const codigoOculto = 'RUL' + Date.now().toString(36).toUpperCase() + Math.random().toString(36).slice(2, 6).toUpperCase();
    const expira = new Date(Date.now() + 10 * 60 * 1000).toISOString().slice(0, 19).replace('T', ' ');
    db.prepare('INSERT INTO discount_codes (codigo, porcentaje, activo, expira, generado_por_ruleta) VALUES (?, ?, 1, ?, 1)')
      .run(codigoOculto, segmento.porcentaje, expira);
    respuesta.codigo = codigoOculto;
    respuesta.porcentaje = segmento.porcentaje;
    respuesta.expira = expira;
    respuesta.segundos_restantes = 600;
  }
...
function getSettings() {
  ...
  return {
    ...
    ruleta: { ...db.DEFAULT_SETTINGS.ruleta, ...(saved.ruleta || {}) },
...
app.put('/api/admin/settings', requireAdmin, uploadBranding.any(), (req, res) => {
  ...
  // Banners intercalados entre productos (imagen completa, sin texto ni opacidad)
  const bannersIntercaladosMeta = b.banners_intercalados_json ? JSON.parse(b.banners_intercalados_json) : current.banners_intercalados;
  const bannersIntercalados = bannersIntercaladosMeta.map((bi, i) => {
    const fileKey = `banner_intercalado_image_${i}`;
    return {
      imagen: fileByField[fileKey] ? `/branding/${fileByField[fileKey].filename}` : (bi.imagen || ''),
      posicion: parseInt(bi.posicion) || 1,
      link: bi.link || ''
    };
  });
  ...
  const updated = {
    ...
    banners_intercalados: bannersIntercalados,
    ...
    ruleta: b.ruleta_json ? JSON.parse(b.ruleta_json) : current.ruleta,
    banners
  };
