...
app.post('/api/admin/popups/:id/items', requireAdmin, uploadBranding.single('imagen'), (req, res) => {
  const { titulo, texto, icono, animacion } = req.body;
  ...
  const info = db.prepare(`
    INSERT INTO popup_items (popup_id, titulo, texto, imagen, icono, animacion, orden) VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(req.params.id, titulo || '', texto || '', imagen, icono || '', animacion || 'none', maxOrden + 1);
  res.json({ ok: true, id: info.lastInsertRowid });
});
...
app.put('/api/admin/popups/items/:itemId', requireAdmin, uploadBranding.single('imagen'), (req, res) => {
  ...
  db.prepare('UPDATE popup_items SET titulo=?, texto=?, imagen=?, icono=?, animacion=? WHERE id=?').run(
    titulo ?? existing.titulo, texto ?? existing.texto, imagen,
    icono ?? existing.icono, animacion ?? existing.animacion, req.params.itemId
  );
  res.json({ ok: true });
});
